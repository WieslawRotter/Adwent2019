...
impl Ampli {
    ...
    fn set_puzzle(&mut self, puzzle: &Vec<i64>) {
        let mut key = 0;
        for digit in puzzle {
            self.set_val(key, *digit);
            key += 1;
        }
    }
    ...
    fn get_val(&self, key: u64) -> i64 {
        if self.puzzle.contains_key(&key) {
            *(self.puzzle.get(&key).unwrap())
        } else {
            0
        }
    }

    fn set_val(&mut self, key: u64, val: i64) {
        
        self.puzzle.insert(key, val);
    }

    fn run_comp(&mut self) -> bool {
        while self.get_val(self.index) != 99 {
            let inst_1 = self.get_val(self.index) % 100;
            let inst_2 = (self.get_val(self.index) / 100) % 10;
            let inst_3 = (self.get_val(self.index) / 1000) % 10;
            let inst_4 = self.get_val(self.index) / 10000;
            let idx_1 = if inst_2 == 1 {
                self.index + 1
            } else if inst_2 == 2 {
                (self.get_val(self.index + 1) + self.rel_base) as u64
            } else {
                (self.get_val(self.index + 1)) as u64
            };
            let idx_2 = if inst_3 == 1 {
                self.index + 2
            } else if inst_3 == 2 {
                (self.get_val(self.index + 2) + self.rel_base) as u64
            } else {
                (self.get_val(self.index + 2)) as u64
            };
            let idx_3 = if inst_4 == 1 {
                self.index + 3
            } else if inst_4 == 2 {
                (self.get_val(self.index + 3) + self.rel_base) as u64
            } else {
                (self.get_val(self.index + 3)) as u64
            };

            if inst_1 == 9 {
                self.rel_base += self.get_val(idx_1);
                self.index += 2;
            } else if inst_1 == 8 {
                if self.get_val(idx_1) == self.get_val(idx_2) {
                    self.set_val(idx_3, 1);
                } else {
                    self.set_val(idx_3, 0);
                };
                self.index += 4;
            } else if inst_1 == 7 {
                if self.get_val(idx_1) < self.get_val(idx_2) {
                ...
                if self.get_val(idx_1) == 0 {
                    self.index = self.get_val(idx_2) as u64;
                } else {
                    self.index += 3;
                }
            } else if inst_1 == 5 {
                ...
                self.output = self.get_val(idx_1);
                println!("{}", self.output);
                self.index += 2;
            } else if inst_1 == 3 {
                ...
                self.set_val(idx_1, self.input.peek().unwrap());
                self.input.remove();
                self.index += 2;
            } else if inst_1 == 2 {
                let new_value = self.get_val(idx_1) * self.get_val(idx_2);
                self.set_val(idx_3, new_value);
                self.index += 4;
            } else if inst_1 == 1 {
                let new_value = self.get_val(idx_1) + self.get_val(idx_2);                
                self.set_val(idx_3, new_value);
